...
const Recipedata = () => {
    ...
    useEffect(() => {
        const fetchRecipe = async () => {
            setLoading(true);
            try {
                const recipesRef = ref(db, `recipes/${type}`);
                const snapshot = await get(recipesRef);
                if (snapshot.exists()) {
                    const data = snapshot.val();
                    const recipesArray = Object.entries(data).map(([key, value]) => ({
                        ...value,
                        firebaseKey: key
                    }));
                    const selectedRecipe = recipesArray.find(r =>
                        r.firebaseKey === id ||
                    ...
                    if (selectedRecipe) {
                        setRecipe({ ...selectedRecipe, id: selectedRecipe.firebaseKey });
                    }
                }
            } catch (error) {
