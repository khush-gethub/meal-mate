...
const RecipeManagement = () => {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        Object.entries(data).forEach(([category, categoryObj]) => {
          const arr = Object.keys(categoryObj).map(k => ({
            ...categoryObj[k],
            _id: k,
            id: k,
            type: category
  ...
  const handleAddRecipe = async (e) => {
    ...
    try {
      ...
      const categoryRef = ref(db, `recipes/${newRecipe.type}`);
      const newRecipeRef = push(categoryRef);
  ...
  const handleUpdateRecipe = async (e) => {
    ...
    try {
      ...
      const recipeRef = ref(db, `recipes/${currentRecipe.type}/${currentRecipe._id}`);
  ...
  const handleDeleteRecipe = async (recipeId, recipeType) => {
    if (window.confirm('Are you sure you want to delete this culinary masterpiece?')) {
      try {
        const recipeRef = ref(db, `recipes/${recipeType}/${recipeId}`);
