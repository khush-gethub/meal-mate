...
const MealPlanner = () => {
    ...
    const fetchPlan = async () => {
        ...
        try {
            if (!user?.uid) return;
            const planRef = ref(db, `mealPlans/${user.uid}`);
            const snapshot = await get(planRef);

            if (snapshot.exists()) {
                const data = snapshot.val();
                if (data.weekDays) {
                    setPlan(data.weekDays);
                    toast.success("Meal plan loaded!");
                }
            } else {
                setPlan(INITIAL_PLAN);
                toast.info("No saved meal plan found. Start planning!");
            }
        } catch (error) {
    ...
    const handleSelectRecipe = (recipe) => {
        ...
        if (day && type) {
            ...
            newPlan[day][type] = {
                ...
                image: recipe.image || '',
                category: recipe.type || recipe.category || 'general'
            };
            setPlan(newPlan);
    ...
    const handleSavePlan = async () => {
        ...
        try {
            if (!user?.uid) return;
            const planRef = ref(db, `mealPlans/${user.uid}`);
            await set(planRef, { weekDays: plan, userId: user.uid, lastUpdated: new Date().toISOString() });
