      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          Object.entries(data).forEach(([category, categoryObj]) => {
            const arr = Object.entries(categoryObj).map(([k, v]) => ({
              ...v,
              _id: k,
              firebaseKey: k
            }));
          ...
          const foundRecipe = allRecipes.find(rec => rec.firebaseKey === id || rec._id === id || rec.id?.toString() === id.toString() || `recipe_${rec.id}` === id);
          if (foundRecipe) {
