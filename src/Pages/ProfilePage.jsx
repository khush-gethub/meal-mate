...
const ProfilePage = () => {
            ...
            try {
                if (user?.uid) {
                    ...
                    if (snap.exists()) {
                        ...
                        setProfileData(prev => ({
                            ...prev,
                            rname: userData.name || user.displayName || '',
    ...
    const fetchMyRecipes = async () => {
        ...
        try {
            ...
            if (snapshot.exists()) {
                ...
                const myRecipesFiltered = myArr.filter(r => r.createdBy === user.uid);
    ...
    const handleProfileUpdate = async (e) => {
        ...
        try {
            if (profileData.password && auth.currentUser) {
                await updatePassword(auth.currentUser, profileData.password);
            }
            if (user?.uid) {
                const userRef = ref(db, `users/${user.uid}`);
                await update(userRef, { name: profileData.rname });
            }
