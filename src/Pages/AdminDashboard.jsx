  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const calculatedTotalUsers = usersSnap.exists() ? Object.keys(usersSnap.val()).length : 0;

        let calculatedTotalRecipes = 0;
        if (recipesSnap.exists()) {
          const recipesData = recipesSnap.val();
          Object.values(recipesData).forEach(cat => {
            calculatedTotalRecipes += Object.keys(cat).length;
          });
        }

        setTotalUsers(calculatedTotalUsers);
        setTotalRecipes(calculatedTotalRecipes);
      } catch (err) {
